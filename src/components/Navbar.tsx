  ...
  return (
    <header 
      className={`fixed top-0 w-full z-50 transition-all duration-300 border-b ${
        scrolled 
          ? "bg-[#050505]/95 backdrop-blur-md shadow-lg border-white/10"
          ...
          <div className="flex-shrink-0">
            <Link href="/" className="flex items-center gap-2" onClick={() => setIsOpen(false)}>
              <Image 
                ...
                width={150} 
                height={50} 
                className={`w-50 object-contain hover:brightness-110 transition-all duration-300 ${scrolled ? "h-7" : "h-9"}`}
                priority
              />
            </Link>
          </div>
